...
### Fixed
- Security and reliability fixes for the Go server, the Python wrapper and CI. They run on the current dependency set. ([#50](https://github.com/SCGIS-Wales/helm-mcp/pull/50))
- The dependency upgrades (Go 1.27, Helm 4.3, k8s 0.37, FastMCP 4 and the pinned tooling) have moved to a separate PR. With them included, this PR's integration job went red while the same job stayed green on `main`, so they need investigating on their own. ([#50](https://github.com/SCGIS-Wales/helm-mcp/pull/50))
